...
@interface SPullDownMenuView ()<UITableViewDelegate, UITableViewDataSource> {
    UIColor     *_menuColor;
...
- (instancetype)initWithFrame:(CGRect)frame
                    withTitle:(NSArray *)titleArray
              withSelectColor:(UIColor *)color {
    ...
    if (self) {
        ...
        _numOfMenu = titleArray.count;//title 的个数
        ...
        for (int i = 0; i < _numOfMenu; i++) {
            ...
            btn.backgroundColor = [UIColor whiteColor];//uibutton的颜色
        ...
        _backgroudView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.3];
...
- (void)animationWithTableViewWithBtn:(SMenuButton *)sender {
    ...
    _backgroudView.frame = CGRectMake(0, CGRectGetMaxY(_tableView.frame), SScreen_Width, SScreen_Height - CGRectGetMaxY(_tableView.frame));
    [_tableView reloadData];
    [self.superview addSubview:_backgroudView];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if ([cell.textLabel.text isEqualToString:[_titleArray objectAtIndex:_currentTitleIndex]]) {
